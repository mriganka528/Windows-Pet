...
A customizable kawaii cartoon-animal desktop companion for **Windows**. It roams
...
it trots over to the toast on four legs, and in auto-close mode swats it shut with
...
part of the app still works *except* noticing notifications, so nothing looks
...
work. If you started the app *before* installing the SDK, install it and run
...
Closing is done by invoking the toast's *own* dismiss control through UI
...
| `npm run watcher:build` | Compile the native watcher explicitly (Debug), with full error output. Use this to see *why* a build failed. |
...
2. **You're paused, or in the wrong mode.** Pause suppresses everything. In *Nudge*
   mode the pet reacts but won't close toasts; only *Auto-close* closes them.
...
process name — **never** the title or body. It reads some control *labels* while
